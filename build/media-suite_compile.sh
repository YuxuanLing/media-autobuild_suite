...
if [[ $jo = y ]] &&
    do_vcs "/e/work/home_repos/ffmpeg_build_windows/jo.git"; then
    do_autoreconf
...
if [[ $jq = y ]] &&
    do_vcs "/e/work/home_repos/ffmpeg_build_windows/jq.git"; then
...
if [[ $dssim = y ]] &&
    do_vcs "/e/work/home_repos/ffmpeg_build_windows/dssim.git"; then
    do_uninstall "${_check[@]}"
...
if { enabled libxml2 || [[ $cyanrip = y ]]; } &&
    do_vcs "/e/work/home_repos/ffmpeg_build_windows/libxml2.git";then
...
if [[ $mplayer = y || $mpv = y ]] ||
    ...
    if do_vcs "/e/work/home_repos/ffmpeg_build_windows/freetype.git"; then
        do_autogen
    ...
    if enabled_any {lib,}fontconfig &&
        do_vcs "/e/work/home_repos/ffmpeg_build_windows/fontconfig.git"; then
    ...
    if do_vcs "/e/work/home_repos/ffmpeg_build_windows/harfbuzz.git"; then
    ...
    if do_vcs "/e/work/home_repos/ffmpeg_build_windows/fribidi.git"; then
    ...
    if do_vcs "/e/work/home_repos/ffmpeg_build_windows/libass.git"; then
        do_autoreconf
...
if [[ $curl = libressl ]] || { [[ $ffmpeg != no ]] && enabled libtls; }; then
    ...
    if do_vcs "/e/work/home_repos/ffmpeg_build_windows/portable.git" libressl; then
...
if [[ $mediainfo = y || $bmx = y || $curl != n || $cyanrip = y ]] &&
    do_vcs "/e/work/home_repos/ffmpeg_build_windows/curl.git"; then
...
if { { [[ $ffmpeg != no || $standalone = y ]] && enabled libtesseract; } ||
    ...
    if do_vcs "/e/work/home_repos/ffmpeg_build_windows/FreeGLUT.git" freeglut; then
    ...
    if do_vcs "/e/work/home_repos/ffmpeg_build_windows/libtiff.git"; then
...
if [[ $ffmpeg != no || $standalone = y ]] && enabled libwebp &&
    do_vcs "/e/work/home_repos/ffmpeg_build_windows/libwebp"; then
    ...
    do_local_patch "./0001-WEBP_DEP_LIBRARIES-use-Threads-Threads.patch" am
    do_local_patch "./0002-deps.cmake-unroll-img-loop-and-use-import-libraries-.patch" am
    do_local_patch "./0003-CMake-link-imageioutil-to-exampleutil-after-defined.patch" am
    do_local_patch "./0004-CMake-use-target_include_directories-instead-of-incl.patch" am
    do_local_patch "./0005-CMake-use-import-libraries-if-possible-for-vwebp.patch" am
    do_local_patch "./0006-CMake-use-import-library-for-SDL-if-available.patch" am
    do_local_patch "./0007-CMake-include-src-along-with-binary_dir-src.patch" am
    do_local_patch "./0008-CMake-add-WEBP_BUILD_WEBPMUX-to-list-of-checks-for-e.patch" am
    do_local_patch "./0009-CMake-add-WEBP_BUILD_WEBPINFO-to-list-of-checks-for-.patch" am
    do_local_patch "./0010-deps-use-pkg-config-instead-of-find_package.patch" am
...
if [[ $ffmpeg != no && -f $opencldll ]] && enabled opencl; then
    ...
    _check=(CL/cl.h)
    if do_vcs "/e/work/home_repos/ffmpeg_build_windows/OpenCL-Headers.git"; then
...
if [[ $ffmpeg != no || $standalone = y ]] && enabled libtesseract; then
    ...
    if do_vcs "/e/work/home_repos/ffmpeg_build_windows/leptonica.git"; then
    ...
    if do_vcs "/e/work/home_repos/ffmpeg_build_windows/tesseract.git"; then
...
if { { [[ $ffmpeg != no ]] && enabled librubberband; } ||
    ...
    do_vcs "/e/work/home_repos/ffmpeg_build_windows/rubberband.git"; then
    do_uninstall "${_check[@]}"
...
if [[ $ffmpeg != no ]] && enabled libzimg &&
    do_vcs "/e/work/home_repos/ffmpeg_build_windows/zimg.git"; then
    do_uninstall "${_check[@]}"
...
if [[ $ffmpeg != no ]] && enabled libilbc &&
    do_vcs "/e/work/home_repos/ffmpeg_build_windows/libilbc.git"; then
    do_uninstall "${_check[@]}"
...
if [[ $standalone = y ]] && enabled libspeex &&
    ...
    do_vcs "/e/work/home_repos/ffmpeg_build_windows/speex.git"; then
...
if [[ $flac = y ]] && do_vcs "/e/work/home_repos/ffmpeg_build_windows/flac.git"; then
...
if { [[ $ffmpeg != no ]] && enabled libfdk-aac; } || [[ $fdkaac = y ]]; then
    ...
    if do_vcs "/e/work/home_repos/ffmpeg_build_windows/fdk-aac"; then
        do_autoreconf
    ...
    _deps=(libfdk-aac.a)
    if [[ $standalone = y ]] &&
        do_vcs "/e/work/home_repos/ffmpeg_build_windows/fdkaac" bin-fdk-aac; then
        do_autoreconf
        do_uninstall "${_check[@]}"
...
if [[ $standalone = y && $faac = y ]] &&
    do_vcs "/e/work/home_repos/ffmpeg_build_windows/faac.git"; then
...
if [[ $exhale = y ]] &&
    do_vcs "/e/work/home_repos/ffmpeg_build_windows/exhale.git"; then
    do_uninstall "${_check[@]}"
    _notrequired=true
...
if [[ $standalone = y ]] && enabled libvorbis &&
    do_vcs "/e/work/home_repos/ffmpeg_build_windows/vorbis-tools.git"; then
...
if enabled libopus && do_vcs "/e/work/home_repos/ffmpeg_build_windows/opus.git"; then
...
if [[ $standalone = y ]] && enabled libopus; then
    ...
    if do_vcs "/e/work/home_repos/ffmpeg_build_windows/opusfile.git"; then
        do_uninstall "${_check[@]}"
    ...
    _deps=(opus.pc)
    if do_vcs "/e/work/home_repos/ffmpeg_build_windows/libopusenc.git"; then
        do_uninstall "${_check[@]}"
        do_autogen
        do_separate_confmakeinstall --disable-{examples,doc}
        do_checkIfExist
    ...
    if do_vcs "/e/work/home_repos/ffmpeg_build_windows/opus-tools.git"; then
...
if [[ $ffmpeg != no ]] && enabled libsoxr &&
    do_vcs "/e/work/home_repos/ffmpeg_build_windows/libsoxr.git"; then
    do_uninstall "${_check[@]}"
...
if [[ $ffmpeg != no ]] && enabled libcodec2; then
    if do_vcs "/e/work/home_repos/ffmpeg_build_windows/codec2.git"; then
...
if [[ $sox = y ]] && do_vcs "/e/work/home_repos/ffmpeg_build_windows/libsndfile.git" sndfile; then
...
if [[ $ffmpeg != no ]] && enabled libopenmpt &&
    do_vcs "/e/work/home_repos/ffmpeg_build_windows//openmpt.git"; then
...
if [[ $ffmpeg != no ]] && enabled libmysofa &&
    do_vcs "/e/work/home_repos/ffmpeg_build_windows/libmysofa.git"; then
    do_uninstall "${_check[@]}"
...
if enabled libflite && do_vcs "/e/work/home_repos/ffmpeg_build_windows/flite.git"; then
...
if { { [[ $ffmpeg != no ]] &&
    ...
    do_vcs "/e/work/home_repos/ffmpeg_build_windows/openal-soft.git"; then
    do_uninstall "${_check[@]}"
...
if { [[ $rtmpdump = y ]] ||
    ...
    do_vcs "/e/work/home_repos/ffmpeg_build_windows/rtmpdump.git" librtmp; then
...
if [[ $vpx = y ]] && do_vcs "/e/work/home_repos/ffmpeg_build_windows/libvpx" vpx; then
    extracommands=()
...
if [[ $ffmpeg != no ]] && enabled libvmaf &&
    do_vcs "/e/work/home_repos/ffmpeg_build_windows/vmaf.git"; then
...
if { [[ $aom = y ]] || [[ $libavif = y ]] || { [[ $ffmpeg != no ]] && enabled libaom; }; } &&
    do_vcs "/e/work/home_repos/ffmpeg_build_windows/aom"; then
    extracommands=()
    if $_aom_bins; then
...
if { [[ $dav1d = y ]] || [[ $libavif = y ]] || { [[ $ffmpeg != no ]] && enabled libdav1d; }; } &&
    do_vcs "/e/work/home_repos/ffmpeg_build_windows/dav1d.git"; then
...
if { enabled librav1e || [[ $libavif = y ]]; } &&
    do_vcs "/e/work/home_repos/ffmpeg_build_windows/cargo-c.git"; then
...
if { [[ $rav1e = y ]] || [[ $libavif = y ]] || enabled librav1e; } &&
    do_vcs "/e/work/home_repos/ffmpeg_build_windows/rav1e.git"; then
...
if [[ $libavif = y ]] && {
    ...
    do_vcs "/e/work/home_repos/ffmpeg_build_windows/libavif.git"; then
    do_uninstall "${_check[@]}"
...
if [[ $jpegxl = y ]] && do_vcs "/e/work/home_repos/ffmpeg_build_windows/jpeg-xl.git"; then
    do_uninstall "${_check[@]}"
...
if { [[ $other265 = y ]] || { [[ $ffmpeg != no ]] && enabled libkvazaar; }; } &&
    do_vcs "/e/work/home_repos/ffmpeg_build_windows/kvazaar.git"; then
...
if { { [[ $ffmpeg != no ]] &&
    ...
    mpv_enabled sdl2; } &&
    do_vcs "/e/work/home_repos/ffmpeg_build_windows/SDL.git"; then
...
if { [[ $mplayer = y ]] || mpv_enabled dvdnav; } &&
    do_vcs "/e/work/home_repos/ffmpeg_build_windows/libdvdread.git" dvdread; then
    do_autoreconf
...
_deps=(libdvdread.a)
if { [[ $mplayer = y ]] || mpv_enabled dvdnav; } &&
    do_vcs "/e/work/home_repos/ffmpeg_build_windows/libdvdnav.git" dvdnav; then
    do_autoreconf
...
if { [[ $ffmpeg != no ]] && enabled libbluray; } || ! mpv_disabled libbluray; then
    ...
    if do_vcs "/e/work/home_repos/ffmpeg_build_windows/libaacs.git"; then
    ...
    if do_vcs "/e/work/home_repos/ffmpeg_build_windows/libbdplus.git"; then
...
if { { [[ $ffmpeg != no ]] && enabled libbluray; } || ! mpv_disabled libbluray; } &&
    do_vcs "/e/work/home_repos/ffmpeg_build_windows/libbluray.git"; then
...
if [[ $ffmpeg != no ]] && enabled libxavs && do_pkgConfig "xavs = 0.1." "0.1" &&
    do_vcs "/e/work/home_repos/ffmpeg_build_windows/xavs.git"; then
...
if [[ $bits = 32bit ]]; then
    ...
    do_vcs "/e/work/home_repos/ffmpeg_build_windows/xavs2.git"; then
    cd_safe build/linux
    [[ -f config.mak ]] && log "distclean" make distclean
...
if [[ $bits = 32bit ]]; then
    ...
    do_vcs "/e/work/home_repos/ffmpeg_build_windows/davs2.git"; then
    cd_safe build/linux
    [[ -f config.mak ]] && log "distclean" make distclean
    do_uninstall all "${_check[@]}"
...
if [[ $ffmpeg != no ]] && enabled libuavs3d &&
    do_vcs "/e/work/home_repos/ffmpeg_build_windows/uavs3d.git"; then
    do_cmakeinstall
...
if [[ $mediainfo = y ]]; then
    [[ $curl = openssl ]] && hide_libressl
    _check=(libzen.{a,pc})
    if do_vcs "/e/work/home_repos/ffmpeg_build_windows/ZenLib.git" libzen; then
    ...
    if do_vcs "/e/work/home_repos/ffmpeg_build_windows/MediaInfoLib.git" libmediainfo; then
    ...
    if do_vcs "/e/work/home_repos/ffmpeg_build_windows/MediaInfo.git" mediainfo; then
...
if [[ $ffmpeg != no ]] && enabled libvidstab &&
    do_vcs "/e/work/home_repos/ffmpeg_build_windows/vid.stab.git" vidstab; then
...
if [[ $ffmpeg != no ]] && enabled_any frei0r ladspa; then
    ...
    if do_vcs "/e/work/home_repos/ffmpeg_build_windows/dlfcn-win32.git"; then
        do_uninstall "${_check[@]}"
        do_cmakeinstall
        do_checkIfExist
    fi
    ...
    _check=(frei0r.{h,pc})
    if do_vcs "/e/work/home_repos/ffmpeg_build_windows/frei0r.git"; then
...
if [[ $ffmpeg != no ]] && enabled decklink &&
    do_vcs "/e/work/home_repos/ffmpeg_build_windows/decklink-headers.git"; then
...
if [[ $ffmpeg != no ]] && enabled libmfx &&
    do_vcs "/e/work/home_repos/ffmpeg_build_windows/mfx_dispatch.git" libmfx; then
    do_autoreconf
...
if [[ $ffmpeg != no ]] && { enabled amf || ! disabled_any autodetect amf; } &&
    do_vcs "/e/work/home_repos/ffmpeg_build_windows//AMF.git"; then
...
if [[ $mp4box = y ]] && do_vcs "/e/work/home_repos/ffmpeg_build_windows/gpac.git"; then
...
if [[ $bits = 32bit ]]; then
    ...
    do_vcs "/e/work/home_repos/ffmpeg_build_windows/SVT-HEVC.git"; then
...
if [[ $bits = 32bit ]]; then
    ...
    do_vcs "/e/work/home_repos/ffmpeg_build_windows/SVT-AV1.git"; then
...
if [[ $bits = 32bit ]]; then
    ...
    do_vcs "/e/work/home_repos/ffmpeg_build_windows/SVT-VP9.git"; then
...
if [[ $xvc == y ]] &&
    do_vcs "/e/work/home_repos/ffmpeg_build_windows/xvc.git"; then
    do_uninstall "${_check[@]}"
...
if [[ $x264 != no ]]; then
    ...
    if do_vcs "/e/work/home_repos/ffmpeg_build_windows/x264.git" ||
        ...
        if [[ $standalone = y && $x264 =~ (full|fullv) ]]; then
            ...
            do_vcs "/e/work/home_repos/ffmpeg_build_windows/ffmpeg.git"
            ...
            if do_vcs "/e/work/home_repos/ffmpeg_build_windows/ffms2.git"; then
                do_uninstall "${_check[@]}"
        ...
        if [[ $standalone = y ]]; then
            ...
            if do_vcs "/e/work/home_repos/ffmpeg_build_windows/l-smash.git" liblsmash; then
                [[ -f config.mak ]] && log "distclean" make distclean
                do_uninstall "${_check[@]}"
                create_build_dir
...
if [[ ! $x265 = n ]] && do_vcs "/e/work/home_repos/ffmpeg_build_windows/x265_git.git"; then
...
if enabled libxvid && [[ $standalone = y ]] &&
    do_vcs "/e/work/home_repos/ffmpeg_build_windows/xvid.git"; then
...
if [[ $ffmpeg != no ]] && { enabled ffnvcodec ||
    ...
    do_vcs "/e/work/home_repos/ffmpeg_build_windows/nv-codec-headers.git" ffnvcodec; then
    do_makeinstall PREFIX="$LOCALDESTDIR"
    do_checkIfExist
fi
...
if enabled libsrt && do_vcs "/e/work/home_repos/ffmpeg_build_windows/srt.git"; then
    do_pacman_install openssl
    hide_libressl
...
if enabled librist && do_vcs "/e/work/home_repos/ffmpeg_build_windows/librist.git"; then
...
if [[ $ffmpeg != no ]] && enabled liblensfun &&
    ...
    do_vcs "/e/work/home_repos/ffmpeg_build_windows/lensfun.git"; then
...
if [[ $bits = 64bit && $vvc = y ]] &&
    do_vcs "/e/work/home_repos/ffmpeg_build_windows/VVCSoftware_VTM.git" vvc; then
...
if [[ $ffmpeg != no ]] && enabled avisynth &&
    do_vcs "/e/work/home_repos/ffmpeg_build_windows/AviSynthPlus.git"; then
    do_uninstall "${_check[@]}"
...
if { { [[ $ffmpeg != no ]] && enabled vulkan; } || ! mpv_disabled vulkan; } &&
    do_vcs "/e/work/home_repos/ffmpeg_build_windows/Vulkan-Loader.git" vulkan-loader; then
...
if [[ $ffmpeg != no ]] && enabled libglslang &&
    do_vcs "/e/work/home_repos/ffmpeg_build_windows/glslang.git"; then
    do_uninstall "${_check[@]}"
...
if [[ $ffmpeg != no ]]; then
    ...
    if do_vcs "/e/work/home_repos/ffmpeg_build_windows/ffmpeg.git"; then
...
if [[ $mpv != n ]] && pc_exists libavcodec libavformat libswscale libavfilter; then
    if ! mpv_disabled lua && opt_exists MPV_OPTS "--lua=5.1"; then
        ...
        do_vcs "/e/work/home_repos/ffmpeg_build_windows/LuaJIT.git" luajit; then
    ...
    if ! mpv_disabled javascript &&
        do_vcs "/e/work/home_repos/ffmpeg_build_windows/mujs.git"; then
    ...
    if mpv_enabled mruby && do_vcs "/e/work/home_repos/ffmpeg_build_windows/mruby.git"; then
    ...
    if ! mpv_disabled shaderc &&
        do_vcs "/e/work/home_repos/ffmpeg_build_windows/shaderc.git"; then
    ...
    if ! mpv_disabled spirv-cross &&
        do_vcs "/e/work/home_repos/ffmpeg_build_windows/SPIRV-Cross.git"; then
    ...
    if do_vcs "/e/work/home_repos/ffmpeg_build_windows/mpv.git"; then
        hide_conflicting_libs
        create_ab_pkgconfig
...
if [[ $bmx = y ]]; then
    ...
    if do_vcs "/e/work/home_repos/ffmpeg_build_windows/libmxf.git" libMXF-1.0; then
    ...
    _deps=(libMXF-1.0.a)
    if do_vcs "/e/work/home_repos/ffmpeg_build_windows/libmxfpp.git" libMXF++-1.0; then
        do_autogen
        do_uninstall include/libMXF++-1.0 "${_check[@]}"
        do_separate_confmakeinstall video --disable-examples
        do_checkIfExist
    ...
    if do_vcs "/e/work/home_repos/ffmpeg_build_windows/bmx.git"; then
        do_autogen
...
if [[ $cyanrip = y ]]; then
    ...
    if do_vcs "/e/work/home_repos/ffmpeg_build_windows/neon.git"; then
    ...
    if do_vcs "/e/work/home_repos/ffmpeg_build_windows/libmusicbrainz.git"; then
    ...
    if do_vcs "/e/work/home_repos/ffmpeg_build_windows/cyanrip.git"; then
        old_PKG_CONFIG_PATH=$PKG_CONFIG_PATH
        _check=("$LOCALDESTDIR"/opt/cyanffmpeg/lib/pkgconfig/libav{codec,format}.pc)
        if flavor=cyan do_vcs "/e/work/home_repos/ffmpeg_build_windows/ffmpeg.git"; then
...
if [[ $vlc == y ]]; then
    ...
    if do_vcs "/e/work/home_repos/ffmpeg_build_windows/fxc2.git"; then
        do_uninstall "${_check[@]}"
...
EOF
    ...
    if do_vcs "/e/work/home_repos/ffmpeg_build_windows/libspatialaudio.git"; then
    ...
    if do_vcs "/e/work/home_repos/ffmpeg_build_windows/icecast-libshout.git" libshout; then
        do_uninstall "${_check[@]}"
    ...
    if do_vcs "/e/work/home_repos/ffmpeg_build_windows/protobuf.git"; then
    ...
    if do_vcs "/e/work/home_repos/ffmpeg_build_windows/pixman.git"; then
    ...
    if do_vcs "/e/work/home_repos/ffmpeg_build_windows/medialibrary.git"; then
    ...
    if do_vcs "/e/work/home_repos/ffmpeg_build_windows/libthai.git"; then
    ...
    if do_vcs "/e/work/home_repos/ffmpeg_build_windows/libebml.git"; then
    ...
    if do_vcs "/e/work/home_repos/ffmpeg_build_windows/libmatroska.git"; then
    ...
    if do_vcs "/e/work/home_repos/ffmpeg_build_windows/vlc.git"; then
...
if [[ $redshift = y ]] && do_vcs "/e/work/home_repos/ffmpeg_build_windows/redshift.git"; then
